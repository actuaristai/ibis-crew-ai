"""Frontend streamlit for playground."""
...
def initialize_session_state() -> None:
    ...
    if 'user_chats' not in st.session_state:
        ...
        st.session_state.session_db = LocalChatMessageHistory(session_id=st.session_state['session_id'],
                                                              user_id=st.session_state['user_id'])
        st.session_state.user_chats = st.session_state.session_db.get_all_conversations()
        st.session_state.user_chats[st.session_state['session_id']] = {'title': EMPTY_CHAT_NAME,
                                                                       'messages': []}
...
def display_message_buttons(message: dict[str, Any], index: int, col1: Any, col2: Any, col3: Any) -> None:  # noqa: ANN401
...
def display_tool_output(tool_call_input: dict[str, Any], tool_call_output: dict[str, Any]) -> None:
...
def handle_user_input(side_bar: SideBar) -> None:
    ...
    if prompt:
        st.session_state.modified_prompt = None
        parts = get_parts_from_files(upload_gcs_checkbox=st.session_state.checkbox_state,
                                     uploaded_files=side_bar.uploaded_files,
                                     gcs_uris=side_bar.gcs_uris)
        st.session_state['gcs_uris_to_be_sent'] = ''
        parts.append({'type': 'text', 'text': prompt})
        st.session_state \
            .user_chats[st.session_state['session_id']]['messages'] \
            .append(HumanMessage(content=parts).model_dump())

        display_user_input(parts)
        generate_ai_response(remote_agent_engine_id=side_bar.remote_agent_engine_id,
...
def generate_ai_response(remote_agent_engine_id: str | None = None,
    ...
    with ai_message:
        ...
        client = Client(remote_agent_engine_id=remote_agent_engine_id,
...
def update_chat_title() -> None:
    ...
    if (st.session_state.user_chats[st.session_state['session_id']]['title'] == EMPTY_CHAT_NAME):
        st.session_state.session_db.set_title(st.session_state.user_chats[st.session_state['session_id']])
    ...
    if st.session_state.run_id is not None:
        ...
        if feedback is not None:
            client = Client(remote_agent_engine_id=side_bar.remote_agent_engine_id,
                            ...
                            url=side_bar.url_input_field,
                            authenticate_request=side_bar.should_authenticate_request)
            client.log_feedback(feedback_dict=feedback,
