"""This module provides a client for streaming events from a server and a stream handler for updating a Streamlit."""
...
@st.cache_resource
def get_remote_agent(remote_agent_engine_id: str) -> Any:  # noqa: ANN401
...
@st.cache_resource
def get_remote_url_config(url: str, authenticate_request: bool) -> dict[str, Any]:  # noqa: FBT001
...
@st.cache_resource()
def get_local_agent(agent_callable_path: str) -> Any:  # noqa: ANN401
...
class Client:
    ...
    def __init__(self,
                 ...
                 url: str | None = None,
                 authenticate_request: bool = False) -> None:  # noqa: FBT001, FBT002
        ...
        Returns:
            None
        Raises:
            ValueError: If neither agent_callable_path nor remote_agent_engine_id is provided
        """
        if url:
            ...
            if agent_callable_path is None:
                msg = 'agent_callable_path cannot be None'
    ...
    def log_feedback(self, feedback_dict: dict[str, Any], run_id: str) -> None:
        ...
        if self.url:
            ...
            msg = 'No agent or URL configured for feedback logging'
            raise ValueError(msg)

    def stream_messages(self, data: dict[str, Any]) -> Generator[dict[str, Any], None, None]:
        ...
        if self.url:
            ...
            if self.authenticate_request:
                headers['Authorization'] = f'Bearer {self.id_token}'
            with requests.post(self.url, json=data, headers=headers, stream=True, timeout=60) as response:
                for line in response.iter_lines():
                    if line:
                        try:
                            ...
                            logger.error(f"Failed to parse event: {line.decode('utf-8')}")
        elif self.agent is not None:
...
class StreamHandler:
    ...
    def __init__(self, st: Any, initial_text: str = '') -> None:  # noqa: ANN401
...
class EventProcessor:
    ...
    def __init__(self, st: Any, client: Client, stream_handler: StreamHandler) -> None:  # noqa: ANN401
    ...
    def process_events(self) -> None:
        ...
        messages = self.st.session_state.user_chats[self.st.session_state['session_id']]['messages']
        ...
        for message, _ in stream:
            if isinstance(message, dict) and message.get('type') == 'constructor':
                message = message['kwargs']  # noqa: PLW2901
        ...
        if self.final_content:
            final_message = AIMessage(content=self.final_content,
                                      id=self.current_run_id,
                                      additional_kwargs=self.additional_kwargs).model_dump()
...
def get_chain_response(st: Any, client: Client, stream_handler: StreamHandler) -> None:  # noqa: ANN401
